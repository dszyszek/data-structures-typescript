...
class LinkedList implements ILinkedList {
    ...
    private placeNodeAt(node: ILinkedListNode, index: number): ILinkedListNode {
        const nodeCopy: ILinkedListNode = cloneDeep(node)
        ...
        if (parentNodeAtIndex === currentNodeAtIndex) {
            ...
            this.listHead = nodeCopy
        } else {
            parentNodeAtIndex.next = nodeCopy
        }
    ...
    public getNodeAt(indexOfNode: number, head: ILinkedListNode = this.listHead): ILinkedListNode {
        ...
        if (indexOfNode > this.linkedListLength) {
            throw Error(
                `Out of range error (getNodeAt executed with index: ${indexOfNode}, while Linked List length is: ${this.linkedListLength})`
            )
    ...
    public pop(): ILinkedListNode {
        ...
        const parentToLastNode: ILinkedListNode = this.getNodeAt(
